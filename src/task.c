...
int task_add(sqlite3 *db, int project_id, const char *name) {
    const char *sql = "INSERT INTO tasks (project_id, name, start_time, end_time, is_running) VALUES (?, ?, 0, 0, 0);";
    sqlite3_stmt *stmt;
...
int task_start(sqlite3 *db, int project_id, const char *name) {
    ...
    const char *sql = "INSERT INTO tasks (project_id, name, start_time, end_time, is_running) VALUES (?, ?, ?, 0, 1);";
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "Failed to prepare statement: %s\n", sqlite3_errmsg(db));
...
int task_stop(sqlite3 *db, int project_id, int task_id) {
    ...
    const char *check_sql = "SELECT is_running FROM tasks WHERE id=? AND project_id=?;";
    sqlite3_stmt *stmt;
    ...
    // Stop the task
    const char *sql = "UPDATE tasks SET end_time=?, is_running=0 WHERE id=? AND project_id=?;";
...
void task_list(sqlite3 *db, int project_id) {
    const char *sql = "SELECT id, name, start_time, end_time, is_running FROM tasks WHERE project_id=? ORDER BY id;";
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "Failed to prepare statement: %s\n", sqlite3_errmsg(db));
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        printf("%-5d %-30s %-20s %-20s %-10s\n", id, name, start_buf, end_buf, is_running ? "Running" : "Stopped");
    }
    sqlite3_finalize(stmt);
}
...
void task_report(sqlite3 *db, int project_id) {
    const char *sql = "SELECT name, start_time, end_time FROM tasks WHERE project_id=?;";
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "Failed to prepare statement: %s\n", sqlite3_errmsg(db));
